...
type ScrapeTickerOptions struct {
...
func (scraper *Scraper) ScrapeTickersFromJSON(path string) error {
	data, err := os.ReadFile(path)
	if err != nil {
		return errors.Join(errors.New("failed to read instructions file"), err)
	}

	var inst instructionsJSON
	...
	return scraper.ScrapeTickers(inst.Tickers, start, end, opts)
}

func (scraper *Scraper) ScrapeTickers(symbols []string, start, end time.Time, options ScrapeTickerOptions) error {
	if len(symbols) == 0 {
		return nil
	}
	...
	for i, symbol := range symbols {
		...
		numInserted, numSkipped, err := scraper.ScrapeTicker(symbol, start, end, &options)
...
func (scraper *Scraper) ScrapeTicker(symbol string, start, end time.Time, options *ScrapeTickerOptions) (int, int, error) {
	// Validate input
	if start.After(end) {
